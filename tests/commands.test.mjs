...
test("delegation remains inline and forwards errors without recursive Skill calls", () => {
  const rescue = read("commands/rescue.md");
  const agent = read("agents/codex-rescue.md");
  const runtime = read("skills/codex-cli-runtime/SKILL.md");
  assert.match(rescue, /subagent_type: "codex:codex-rescue"/);
  assert.doesNotMatch(rescue, /^context:\s*fork/m);
  assert.match(rescue, /runtime's `task --background`/);
  assert.match(rescue, /without asking/);
  assert.match(agent, /gpt-6-astra/);
  assert.match(agent, /actionable stderr/);
  assert.doesNotMatch(agent, /gpt-5-4-prompting/);
  assert.match(runtime, /Add `--write` only for requested implementation/);
  for (const name of ["delegate", "models", "steer"]) assert.ok(read("commands/" + name + ".md"));
});
